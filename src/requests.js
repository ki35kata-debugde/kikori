...
function completeRequest(r){
  ...
  maybeAnnounceFavor(r.client);
}
/* その依頼主の依頼を今ちょうど全部終えたら、頼み事が解禁されたことを一度だけ知らせる */
function maybeAnnounceFavor(client){
  const fav=FAVORS[client]; if(!fav)return;
  WORLD.favors[client]??={};
  if(WORLD.favors[client].announced||!favorUnlocked(client))return;
  WORLD.favors[client].announced=true;
  if(typeof announceFavor==='function')announceFavor(client,fav);
}
